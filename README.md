...
**Member writes** (any project member; preview + Confirm in Discord): `create_task`, `draft_task`, `update_task`, `delete_task`, `set_task_status` (Kanban column dropdown), `add_comment`, `link_task_milestone` (milestone dropdown), `log_time`, work session start/pause/resume/stop, `submit_issue_report`, `accept_invitation` / `decline_invitation`, `link_branch`, `create_and_link_branch`, `attach_link`, `stage_build`, `stage_review`.
...
**Reads** execute immediately: `list_projects`, `list_tasks` (supports `column_id` for exact Kanban swimlanes), `list_my_tasks`, `get_task`, `get_kanban_board`, `get_task_timeline`, `project_snapshot`, `project_query`, `list_milestones`, `list_repositories`, `list_project_members`, `list_pending_invitations`, `get_active_session`.
